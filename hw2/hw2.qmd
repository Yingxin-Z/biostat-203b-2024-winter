...
-   According to the output of the codes below, `fread` is the fastest, only using about 0.5s. `read_csv` is the second fastest, using about 1.9s. `read.csv` is the slowest, using about 3.5s.

-   The parsed data types are different for `read_csv`, `read.csv` and `fread`. For example, in the parsed data types for `read_csv`, the `subject_id` is `double`, while in the parsed data types for `fread` and `read.csv`, the `subject_id` is `integer`. In the parsed data types for `fread` and `read_csv`, the `admittime` is `double`, while in the parsed data types for `read.csv`, the `admittime` is `character`.

The parsed data types for `fread` are different from the other two. In detail, the parsed data types for `read_csv` and `read.csv` are all `double`, while the parsed data types for `fread` are `integer` and `double`.
...
**Answer:** The program takes more than 5 minutes on my computer and I aborted the program.
...
**Answer**: The first 10 lines of `labevents_filtered.csv.gz` are displayed below. The number of lines in `labevents_filtered.csv.gz` is 24855909. It takes about 22s for `read_csv` to ingest `labevents_filtered.csv.gz`.
...
**Answer**: The number of lines in `labevents_filtered.csv.gz` is 24855908. It takes about 23s for `read_csv` to ingest `labevents_filtered.csv.gz`.
...
**Answer**: The Parquet file `labevents.parquet` is 116M. The ingest+select+filter process of the Parquet file takes about 0.015s. The number of rows in the result tibble is 24855908. The first 10 rows of the result tibble are the same as those in Q2.3.
...
system.time(arrow::open_dataset("labevents.parquet") |> 
   as_tibble())
```
...
zcat < ~/mimic/icu/chartevents.csv.gz | awk -F, '($7 == 220045 || $7 == 220181 
|| $7 == 220179 || $7 == 223761 || $7 == 220210)' 
