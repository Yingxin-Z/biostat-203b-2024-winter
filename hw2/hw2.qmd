...
- According to the output of the codes below, `fread` is the fastest, only using about 0.5s. 

- The parsed data types are the same for `read_csv` and `read.csv`. The parsed data types for `fread` are different from the other two. In detail, the parsed data types for `read_csv` and `read.csv` are all `double`, while the parsed data types for `fread` are `integer` and `double`.
...
**Answer**: This does not solve the ingestion issue. It still takes more than 2 minutes on my computer.
...
arrow::write_dataset(arrow::open_dataset("labevents.csv", format = 'csv') |> 
                       select(subject_id, itemid, charttime, valuenum) |> 
                       filter(itemid %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)), "labevents.parquet")
```
