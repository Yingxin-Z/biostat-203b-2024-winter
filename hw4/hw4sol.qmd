...
chartevents_tble <- tbl(con_bq, "chartevents") |>
  # only the chart events for the patients in icustays_tble
  semi_join(icustays_tble, by = "subject_id") |>
  # only the chart items listed in HW3
  filter(itemid %in% c(220045, 220179, 220180, 223761, 220210)) |>
  left_join(
    select(icustays_tble, subject_id, stay_id, intime, outtime), 
    by = c("subject_id", "stay_id")) |>
  # only the first chart events during ICU stay
  group_by(subject_id, stay_id, itemid) |>
...
The numerical summary of my tibble after preprocessing is as follows:
