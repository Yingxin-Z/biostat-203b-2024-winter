...
```{bash}
# display the first 2 lines of each file
zcat < ~/mimic/hosp/patients.csv.gz | head -2
zcat < ~/mimic/hosp/admissions.csv.gz | head -2
zcat < ~/mimic/hosp/transfers.csv.gz | head -2
zcat < ~/mimic/hosp/labevents.csv.gz | head -2
zcat < ~/mimic/hosp/procedures_icd.csv.gz | head -2
zcat < ~/mimic/hosp/diagnoses_icd.csv.gz | head -2
zcat < ~/mimic/hosp/d_icd_procedures.csv.gz | head -2
zcat < ~/mimic/hosp/d_icd_diagnoses.csv.gz | head -2
```
...
ggsave("output_plot.png", plot, width = 12, height = 5)
```
