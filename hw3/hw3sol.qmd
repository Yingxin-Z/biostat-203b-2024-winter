...
**Answer:** Here I use the Parquet format `labevents_pq` generated in Homework 2 and draw the visualization for the patient with `subject_id` 10013310.
...
plot <- ggplot(chartevents_d_10013310, 
...
**Answer:** The summary plots of the cohort are shown below.
...
ggplot(data = mimic_icu_cohort) + 
  ...
  labs(title = "Distribution of Length of ICU Stay", 
       x = "Length of ICU Stay (los)", y = "Count") +
  theme_minimal()
```
...
- From the Length of ICU stay `los` vs demographic variables plots below, we can see that most of the patients are white, have Medicare or Other insurance, are married, male, and have an age at intime between 50 and 90. Among all of the groups of these demographic variables, the majority of patients stay in the ICU for less than 2.5 days.
...
- From the Length of ICU stay `los` vs the last available lab measurements before ICU stay plots below, we can see that the relationship between the length of ICU stay and the last available lab measurements before ICU stay is not very clear.
...
- From the Length of ICU stay `los` vs the first vital measurements within the ICU stay plots below, we can see that the relationship between the length of ICU stay and the first vital measurements within the ICU stay is also not very clear and it is hard to find a pattern.
...
- From the Length of ICU stay `los` vs first ICU unit plots below, we can see that Neuro ICU has the lowest number of first-ICU-unit patients. The number of patients who stay in the ICU for less than 2.5 days takes up the majority of all first-ICU-unit types.
